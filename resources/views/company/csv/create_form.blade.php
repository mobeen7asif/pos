...
<div class="row">
    <div class="col-lg-12">
        <section class="panel">
            <header class="panel-heading">{{ isset($submitButtonText) ? $submitButtonText : 'Import' }} Customers
                <span class="tools pull-right">
                            <a href="{{ asset('/download/file/customers.csv') }}" class="btn btn-info btn-sm" data-toggle="tooltip" title="Download Sample File" download>
            ...
            <div class="panel-body">
                <div class="position-center" id="form">
                    ...
                    <div class="form-group {{ $errors->has('csv_file') ? 'has-error' : ''}}">
                        ...
                        <div class="col-md-9">
                            <div class="custom-upload" style="position: relative;width: 17%;     border: 1px solid #aaa;">
                                <img src="{{url('/')}}/upload_icon.png" style="display: block;width: 100%">
                                <input id="image_check" name="csv_file" class="input_media" type="file" style="position: absolute; left: 0; top: 0; width: 100%;height: 100%;z-index: 1;opacity: 0;cursor: pointer">
                            </div>
...
@section('scripts')
    <script type="text/javascript">
        ...
        function checkPendingFile() {
            $.ajax({
                url : "{{url('/company')}}" + '/insert-customers-file',
                type: 'post',
                data: {},
                success: function (result) {
                    if(result.complete_status === 0){
                        $('#stats').html(result.stats);
                        $('.panel-heading').html('Uploading CSV');
                        setTimeout(checkPendingFile, 5000);
                    } else {
                        $('#stats').html(result.stats);
                        toastr.success("Customers Added");
