...
@php
    ...
    if($bogo_type == 'product'){
        foreach($discount->discountBogo as $bogo_discount){
        $selected_product_id_array[] = $bogo_discount->to_id;
        }
    } else {
     foreach($discount->discountBogo as $bogo_discount){
        $selected_category_id_array[] = $bogo_discount->to_id;
        }
    }
...
<div class="row">
    <div class="col-lg-12">
        <section class="panel">
            <header class="panel-heading">{{ isset($submitButtonText) ? $submitButtonText : 'Update' }} Bogo Discount</header>
                ...
                <input name="discount_id" type="hidden" value="{{$discount->id}}">
                 ...
                 <div class="form-group {{ $errors->has('store_id') ? 'has-error' : ''}}">
                    ...
                    <div class="col-md-9">
                        ...
                        {!! Form::text('date_time', $discount->date_time, ['class' => 'form-control datetime','placeholder'=>'Date']) !!}
                        ...
                        <div class="col-md-9">
                            <select name="store_id" class="form-control required select_store">
                                @foreach($stores as $store)
                                    <option @if($store_id == $store->id)) selected @endif value={{$store->id}}>{{$store->name}}</option>
                                @endforeach
                            </select>
                    ...
                    <div class="form-group {{ $errors->has('bogo_type') ? 'has-error' : ''}}">
                        ...
                        <div class="col-lg-9 radio_class">
                            <div>
                                <input @if($bogo_type == 'product') checked @endif class="bogo_type" required type="radio" name="bogo_type" value="product"> Products
                            </div>
                            <div>
                                <input @if($bogo_type == 'category') checked @endif class="bogo_type" required type="radio" name="bogo_type" value="category"> Categories
                            </div>
                    ...
                    <div class="form-group {{ $errors->has('required_product') ? 'has-error' : ''}} required_product_div">
                        ...
                        <div class="col-md-9" id="select_required_product" >
                            @if($bogo_type == 'product')
                            <select name="required_product" class="form-control required required_product ">
                                @foreach($products as $product)
                                    <option @if($discount->discountBogo->first()->from_id == $product->id) selected @endif value="{{$product->id}}">{{$product->name}}</option>
                                @endforeach
                            </select>
                            @endif
                    ...
                    <div class="form-group {{ $errors->has('optional_products') ? 'has-error' : ''}} optional_products_div">
                        ...
                        <div class="col-md-9" id="select_optional_products" >
                            @if($bogo_type == 'product')
                                <select name="optional_products[]" multiple class="form-control required optional_products ">
                                    @foreach($products as $product)
                    ...
                    <div class="form-group {{ $errors->has('required_category') ? 'has-error' : ''}} required_category_div">
                        ...
                        <div class="col-md-9" id="select_required_category" >
                            @if($bogo_type == 'category')
                                <select name="required_category" class="form-control required required_category ">
                                    @foreach($categories as $category)
                                        <option @if($discount->discountBogo->first()->from_id == $category->id) selected @endif value="{{$category->id}}">{{$category->category_name}}</option>
                                    @endforeach
                                </select>
                            @endif
                    ...
                    <div class="form-group {{ $errors->has('optional_categories') ? 'has-error' : ''}} optional_categories_div">
                        {!! Form::label('optional_categories', 'Category To', ['class' => 'col-md-3 control-label']) !!}
                        <div class="col-md-9" id="select_optional_categories" >
                            @if($bogo_type == 'category')
                                <select name="optional_categories[]" multiple class="form-control required optional_categories ">
                                    @foreach($categories as $category)
                                        <option @if(in_array($category->id,$selected_category_id_array)) selected @endif value="{{$category->id}}">{{$category->category_name}}</option>
                                    @endforeach
                                </select>
                            @endif
                    ...
                    <div class="form-group {{ $errors->has('discount_type') ? 'has-error' : ''}}">
                        ...
                        <div class="col-md-9">
                            {!! Form::select('discount_type', $discount_type, $discount->discount_type, ['class' => 'form-control discount_type']) !!}
                            {!! $errors->first('parent_id', '<p class="help-block">:message</p>') !!}
...
@section('scripts')
    ...
    <script>
        ...
        $('.bogo_type').change(function () {
           ...
           if(bogo_type === 'product'){
               //get products
               $("#select_optional_products").html('');
               $("#select_required_product").html('');
               $.ajax({
                   type: "get",
                   url: "{{ url('company/get-products-ajax') }}",
                   data: {store_id:store_id},
                   success:function (result) {
                       var html = '<select name="required_product" class="form-control required required_product ">';
               ...
               $("#select_required_category").html('');
               $("#select_optional_categories").html('');
               $.ajax({
                   type: "get",
